...
class _NPYDataSource(FileDataSource):
    ...
    def collect_files(self):
        ...
        paths_relative = list(map(lambda l: l.decode("utf-8").split("|")[self.col], lines)) # dummy get inserted here
        ...
        #print('paths', paths) # dummy is inserted here don't know why
        paths = [p for p in paths if 'dummy' not in p]
        return paths
...
def train_loop(device, model, data_loaders, optimizer, writer, checkpoint_dir=None):
    ...
    while global_epoch < hparams.nepochs:
        for phase, data_loader in data_loaders.items():
            print('phase', phase, 'len of data_loader', len(data_loader))
            if phase == 'dev':
                continue
            train = (phase == "train_no_dev")
            running_loss = 0.
